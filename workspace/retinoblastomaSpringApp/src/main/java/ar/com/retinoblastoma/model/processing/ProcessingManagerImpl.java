...
public class ProcessingManagerImpl implements ProcessingManager {
	...
	public void CalculateColorsPercentageHSL(Mat img, HistogramHSL histogram) {
		...
		// Double othersPercent= 1.0 -
		// (whitePercent+blackPercent+redPercent+yellowPercent+greenPercent);
		//
		...
		//
		// // for (int c=0;c<3;c++){
		// // Imgproc.calcHist(hsvPlanes, new MatOfInt(c),new Mat(), b_hist,
		// histSize, histRange, accumulate);
		// // Core.normalize(b_hist, b_hist, 3, histImage.rows(),
		// Core.NORM_MINMAX);
		// // for (int i = 1; i < 256; i++) {
		// // Core.line(histImage, new Point(bin_w * (i - 1),hist_h-
		// Math.round(b_hist.get( i-1,0)[0])),
		// // new Point(bin_w * (i), hist_h-Math.round(Math.round(b_hist.get(i,
		// 0)[0]))),
		// // mColorsBGR[c], 2, 8, 0);
		// // }
		// // }
